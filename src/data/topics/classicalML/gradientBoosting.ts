...
export const gradientBoosting: Topic = {
  ...
  content: `
    ...
    <p>Imagine you're predicting house prices. Your first model (a simple average) predicts <code>$300k</code> for all houses, achieving moderate error. The second model doesn't try to predict prices directly; instead, it predicts the errors of the first model—where it overestimated and where it underestimated. If the first model predicted <code>$300k</code> for a house worth <code>$250k</code> (error = <code>-$50k</code>), the second model learns to predict this <code>-$50k</code> error. Adding the first model's prediction (<code>$300k</code>) and the second model's correction (<code>-$50k</code>) gives <code>$250k</code>, closer to truth.</p>
